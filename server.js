...
io.on('connection', (socket) => {
  ...
  socket.on("send", (message) => {
    ...
    if (room === "Server_Updates" && username !== "Server") {
      socket.emit("recieve", "Server: ⚠️ Only users with the valid privileges can send messages in the Server_Updates room");
      return;
    }
    ...
    if (room === "Server_Updates") {
      ...
      if (pin !== "1024") {
        socket.emit("recieve", "Server: ⚠️ Invalid PIN. Please enter the correct PIN followed by your message (e.g., 'XXXX your message')");
        return;
      }
